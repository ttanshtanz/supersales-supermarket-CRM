...
def fetch_orders(user_id):
    ...
    c.execute('''
        SELECT p.product_id, p.product_category, p.product_name, p.product_brand, 
               p.product_weight_g, p.price, c.order_id, c.order_status, u.username, p.image1
        FROM orders c
...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top" tabindex="-1" data-testid="stHeader">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav justify-content-center mx-auto">
      ...
      <li class="nav-item">
        <a class="nav-link active" href="/seller_reviews" target="_self"><i class="fas fa-star"></i> Reviews</a>
      </li>
      <li class="nav-item">
...
with colb:
    with st.container(border=True):
            if delivered_orders:
                st.header("Your Delivered Orders & Reviews")
                st.write(" ")
                for order in delivered_orders:
                    product_id, product_category, product_name, product_brand, product_weight_g, price, order_id, order_status, name, image1, review_score, review_comment_message, sentiment_result = order
                    col1, col2, col3, col4, col5, col6, col7 = st.columns([1, 3, 2, 2, 2, 2, 3])
                    ...
                    if review_score:
                        col6.write(f'**Rating:** {review_score}/5.0')
                        col7.write(f'**Review:** {review_comment_message} ({sentiment_result})')
                    else:
                        col6.write("No review yet")

                    st.markdown("""<hr style="height:1px;border:none;color:#333;background-color:#333;width:99%;" /> """, unsafe_allow_html=True)
            else:
                st.write(" ")
                st.write("You have no delivered orders.")
